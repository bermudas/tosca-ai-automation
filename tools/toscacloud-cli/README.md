...
Settings are stored in `tools/toscacloud-cli/.env` (next to the script).

```bash
python tools/toscacloud-cli/tosca_cli.py config set \
...
Run `python tools/toscacloud-cli/tosca_cli.py identity apps` to list all registered OAuth2 applications in your tenant and find the one intended for API access.

### Generate a client secret

```bash
# Check if secrets already exist
python tools/toscacloud-cli/tosca_cli.py identity secrets <appId>

# Generate a new secret (copy it immediately – shown only once)
python tools/toscacloud-cli/tosca_cli.py identity new-secret <appId>

# Retrieve a secret value by its secretId
python tools/toscacloud-cli/tosca_cli.py identity get-secret <appId> <secretId>
```
...
```bash
python tools/toscacloud-cli/tosca_cli.py config set --space-id your-space-uuid
```
...
| Command | Description |
|---------|-------------|
| `config set [options]` | Save settings to `.env` next to the script |
...
```bash
python tools/toscacloud-cli/tosca_cli.py identity apps
python tools/toscacloud-cli/tosca_cli.py identity secrets <appId>
python tools/toscacloud-cli/tosca_cli.py identity new-secret <appId>
python tools/toscacloud-cli/tosca_cli.py identity get-secret <appId> <secretId>
python tools/toscacloud-cli/tosca_cli.py identity delete-secret <appId> <secretId>
```
...
```bash
# Show full metadata
python tools/toscacloud-cli/tosca_cli.py cases get <caseId>
...
python tools/toscacloud-cli/tosca_cli.py cases steps <caseId> --json

# Clone an existing test case with all steps, values, and config params
python tools/toscacloud-cli/tosca_cli.py cases clone <caseId>
python tools/toscacloud-cli/tosca_cli.py cases clone <caseId> --name "My copy"

# Create a blank test case
python tools/toscacloud-cli/tosca_cli.py cases create --name "My TC" --desc "..." --state Planned
...
python tools/toscacloud-cli/tosca_cli.py cases export-tsu --ids "id1" --block-ids "bid1" --output bundle.tsu

# Import test cases from a .tsu file (undocumented MBT v2 endpoint)
python tools/toscacloud-cli/tosca_cli.py cases import-tsu --file my_export.tsu

# Delete
python tools/toscacloud-cli/tosca_cli.py cases delete <caseId> --force
```
...
python tools/toscacloud-cli/tosca_cli.py blocks add-param <blockId> --name Material4 --desc "4th material"
python tools/toscacloud-cli/tosca_cli.py blocks add-param <blockId> --name Material4Quantity --value-range '1,2,3,4'
...
python tools/toscacloud-cli/tosca_cli.py playlists list --search "smoke" --limit 20
python tools/toscacloud-cli/tosca_cli.py playlists get <playlistId>
...
python tools/toscacloud-cli/tosca_cli.py playlists set-characteristic <playlistId> \
    --name AgentIdentifier --value Tosca-Team-Agent

# Trigger a run (returns RunId immediately)
python tools/toscacloud-cli/tosca_cli.py playlists run <playlistId>
...
python tools/toscacloud-cli/tosca_cli.py playlists run <playlistId> --wait --poll 10

# Private run (not visible to other users)
python tools/toscacloud-cli/tosca_cli.py playlists run <playlistId> --private

# Run status and management
python tools/toscacloud-cli/tosca_cli.py playlists status <runId>
python tools/toscacloud-cli/tosca_cli.py playlists cancel <runId> --reason "Manual stop"
python tools/toscacloud-cli/tosca_cli.py playlists tc-runs <runId>

# JUnit results (JSON format)
python tools/toscacloud-cli/tosca_cli.py playlists results <runId>
python tools/toscacloud-cli/tosca_cli.py playlists results <runId> --save results.json

# Per-unit agent logs (full TBox transcript with .NET stack traces)
python tools/toscacloud-cli/tosca_cli.py playlists logs <runId>
python tools/toscacloud-cli/tosca_cli.py playlists logs <runId> --save ./logs            # save logs.txt + JUnit.xml + TBoxResults.tas + TestSteps.json per unit
python tools/toscacloud-cli/tosca_cli.py playlists logs <executionId> -e --quiet --save ./logs   # input is already an E2G executionId

# List per-unit attachments with their SAS-signed Azure Blob URLs (logs, JUnit, TBoxResults, TestSteps, Recording)
python tools/toscacloud-cli/tosca_cli.py playlists attachments <runId>
python tools/toscacloud-cli/tosca_cli.py playlists attachments <runId> --json

# List all runs in the space
python tools/toscacloud-cli/tosca_cli.py playlists list-runs
python tools/toscacloud-cli/tosca_cli.py playlists list-runs --limit 100

# Delete a run record
python tools/toscacloud-cli/tosca_cli.py playlists delete-run <runId> --force
```
...
python tools/toscacloud-cli/tosca_cli.py inventory search "login"

# Filter by type
python tools/toscacloud-cli/tosca_cli.py inventory search "" --type TestCase --limit 50
python tools/toscacloud-cli/tosca_cli.py inventory search "SAP" --type TestCase

# Scope to a specific folder (client-side folderKey filter)
python tools/toscacloud-cli/tosca_cli.py inventory search "" --type TestCase --folder-id <folderEntityId>

# Include breadcrumb path in results
python tools/toscacloud-cli/tosca_cli.py inventory search "login" --include-ancestors

# Get full inventory record for a specific artifact
python tools/toscacloud-cli/tosca_cli.py inventory get TestCase <entityId>
python tools/toscacloud-cli/tosca_cli.py inventory get Module <entityId>
...
```bash
python tools/toscacloud-cli/tosca_cli.py inventory search "" --type TestCase --json | \
...
python tools/toscacloud-cli/tosca_cli.py inventory move Module <entityId> --folder-id <folderEntityId>
```
...
python tools/toscacloud-cli/tosca_cli.py inventory create-folder --name "Regression"
python tools/toscacloud-cli/tosca_cli.py inventory create-folder --name "Regression" \
    --parent-id "00000000-0000-0000-0000-000000000000" \
    --desc "Regression suites"

# Rename a folder
python tools/toscacloud-cli/tosca_cli.py inventory rename-folder <folderEntityId> --name "New Name"

# Delete a folder (children moved to parent by default — "ungroup")
python tools/toscacloud-cli/tosca_cli.py inventory delete-folder <folderEntityId>
# Delete folder and all its contents recursively
python tools/toscacloud-cli/tosca_cli.py inventory delete-folder <folderEntityId> --delete-children --force

# Show the ancestor chain (breadcrumb) for a folder
python tools/toscacloud-cli/tosca_cli.py inventory folder-ancestors <folderEntityId>

# List the full folder tree (or a subtree)
python tools/toscacloud-cli/tosca_cli.py inventory folder-tree
python tools/toscacloud-cli/tosca_cli.py inventory folder-tree --folder-ids "id1,id2"
```
...
```bash
python tools/toscacloud-cli/tosca_cli.py simulations list
python tools/toscacloud-cli/tosca_cli.py simulations list --tags "regression,api"
python tools/toscacloud-cli/tosca_cli.py simulations get <fileId>
python tools/toscacloud-cli/tosca_cli.py simulations create \
    --name "api-mock.json" \
    --file ./api-mock.json \
    --tags "api,v2" \
    --components "Services,Runnables"
python tools/toscacloud-cli/tosca_cli.py simulations delete <fileId>
```
...
```bash
pip install openai
python tools/toscacloud-cli/tosca_cli.py config set --openai-key sk-...

python tools/toscacloud-cli/tosca_cli.py ask "show me all steps for the successful login test case"
python tools/toscacloud-cli/tosca_cli.py ask "clone the login test case"
python tools/toscacloud-cli/tosca_cli.py ask "run the smoke test playlist and wait"
python tools/toscacloud-cli/tosca_cli.py ask "move test case abc to the regression folder"
python tools/toscacloud-cli/tosca_cli.py ask "export test cases id1 and id2 to a tsu file"
python tools/toscacloud-cli/tosca_cli.py ask "cancel run xyz" --dry-run   # preview without executing
```
