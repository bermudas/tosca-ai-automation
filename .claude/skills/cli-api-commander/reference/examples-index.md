...
## Live-tested workflows (agent-authored)

Verified against live Commander via Remote Control (`TcShellRemoteControl.ps1`) or headless TCShell.

### Create a single-user SQLite workspace (headless)

```powershell
TCShell.exe -newworkspace "C:\TOSCA_Workspaces\MyWorkspace" SQLITE
```

TCShell exits automatically after workspace initialisation. Subsequent operations open the `.tws` file.

### Create a test case

All create tasks produce the object with a **default name** — no input prompt. Set the name immediately after with `set Name`:
...
### Add a manual test step to a test case

```
JumpToNode "/TestCases/FirstTest"
task "Create Manual XTestStep"
set Name "MyStep"
save
```

### Full workflow: create workspace, test case, and test step

```powershell
TCShell.exe -newworkspace "C:\TOSCA_Workspaces\MyWorkspace" SQLITE
TCShell.exe -workspace "C:\TOSCA_Workspaces\MyWorkspace\MyWorkspace.tws" script.tcs
```
