...
Scripts live at `.claude/skills/browser-verify/scripts/` in this repository:
...
The scripts are at `<repo root>/.claude/skills/browser-verify/scripts/`.

```bash
SCRIPTS="$(git rev-parse --show-toplevel 2>/dev/null || pwd)/.claude/skills/browser-verify/scripts"
